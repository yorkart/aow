...
配置机器人后，可在“页面提示”“飞书推送”“微信推送”中多选，启用通知时至少选择一项。保存立即生效；移除机器人前需先取消选择对应推送。IM 推送由服务端处理，网页关闭后仍会发送。飞书和微信配置可分别更新、移除，不会覆盖另一渠道。

IM 凭证和通知设置保存在本机状态目录的 `notification-settings.json`；微信上下文与轮询游标单独保存在 `im/wechat/<binding-id>.json`。文件权限均为 `0600`，不进入 Git 配置仓库；重启后自动恢复设置。文件包含本机明文凭证，请按凭据管理。移除或重新绑定微信会停止旧轮询并删除旧上下文；“解除微信绑定”移除 AoW 本机配置，不代替微信端的账号管理。

自动化任务的“失败提醒”也可选择飞书或微信 Bot，与交互任务完成通知的开关独立。发送渠道取自该次执行的配置快照，之后修改任务不会改变已开始执行的通知渠道。
