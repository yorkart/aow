...
`website/snapshot/data.json` 保存采集时间、仓库地址、完整提交 ID、原始 API 响应和真实 PTY 输出。目前采集范围是提交 `43e0341` 的 225 个源码与文档文件、5 次实际提交及其文件差异。工作台中的项目对应 AoW 仓库，Notes 对应该提交的 `docs/`。项目头像接口也采集真实响应，桌面和手机直接引用返回的头像 URL。
