...
try {
  ...
  async function fixture(t, { bot = false, selected = false, unavailable = false } = {}) {
    ...
    await page.route('**/api/aow/notification-settings', route => route.fulfill({
      ...
      json: unavailable ? { message: 'unavailable' } : { im: { providers: bot ? [{ provider: 'feishu', app_id: 'test', secret_configured: true }] : [] }, notifications: { agent_task_completed: { enabled: false, channels: [] } } },
    }));
    ...
    await page.getByText('正在检查飞书 Bot 配置…', { exact: true }).waitFor({ state: 'hidden' });
    return { page, errors };
  }
  ...
  await test('configured bot can be selected even when interactive notifications are disabled', async t => {
    ...
    assert.equal(await select.locator('option[value="feishu"]').isDisabled(), false);
  ...
  await test('missing bot cannot be newly selected and does not block saving', async t => {
    ...
    await page.getByText('在 Settings → IM 配置飞书 Bot 后，可选择失败提醒。', { exact: true }).waitFor();
  ...
  for (const unavailable of [false, true]) {
    await test(`synced selection survives editing when bot ${unavailable ? 'settings cannot be read' : 'is missing'}`, async t => {
      ...
      await page.getByText(unavailable ? '无法读取飞书 Bot 配置，已有提醒设置仍会保留。' : '当前环境未配置飞书 Bot，已保留提醒设置；配置完成后，后续执行失败时会发送提醒。', { exact: true }).waitFor();
